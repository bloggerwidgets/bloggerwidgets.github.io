...
function podgladaj() {
	...
	elem.setAttribute('class', 'xMostCommented');
...
function generujKod() {
	...
	kod += ' async src="https://cdn.jsdelivr.net/gh/bloggerwidgets/scripts/mostcommentedposts.js"><\/script>';
	return kod;
}
...
pobierzKod.onclick = function() {
	pobierzWidga(kodWidzetu.value, widgetsTitle.value ? widgetsTitle.value : 'Most_Commented_Posts_Widget');
}
...
pobierzWidzeta.onclick = function() {
	if (!this.classList.contains('nieaktywny')) {
		...
		pobierzWidga(ns, nazwaDoZapisu.value ? nazwaDoZapisu.value : 'Most_Commented_Posts_Widget');
	}
}
...
const url = 'https://bloggerwidgets.github.io/most-commented-posts';
const tytul = 'Most Commented Posts Widget For Blogger';
const pedaly = [{
	'nzw' : 'pejsbuk',
	...
	'nzw' : 'goglebokmar',
	'link' : 'https://www.google.com/bookmarks/mark?op=edit&bkmk=' + url + '&title=' + tytul + '&annotation=' + encodeURIComponent('A cool widget to display most commented Blogger posts.') + '&labels=' + encodeURIComponent('blogger, widgets, free, popular, posts')
}, {
	'nzw' : 'dyg',
