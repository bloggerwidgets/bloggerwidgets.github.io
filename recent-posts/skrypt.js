...
function podgladaj() {
	...
	elem.setAttribute('class', 'xRecentPost');
...
pobierzKod.onclick = function() {
	pobierzWidga(kodWidzetu.value, widgetsTitle.value ? widgetsTitle.value : 'Recent_Posts_Blogger_Widget');
}
...
pobierzWidzeta.onclick = function() {
	if (!this.classList.contains('nieaktywny')) {
		...
		pobierzWidga(ns, nazwaDoZapisu.value ? nazwaDoZapisu.value : 'Recent_Posts_Blogger_Widget');
	}
}
...
const url = 'https://bloggerwidgets.github.io/recent-posts';
const tytul = 'Recent Posts Widget For Blogger';
const pedaly = [{
	'nzw' : 'pejsbuk',
	...
	'nzw' : 'goglebokmar',
	'link' : 'https://www.google.com/bookmarks/mark?op=edit&bkmk=' + url + '&title=' + tytul + '&annotation=' + encodeURIComponent('A cool widget to display recent Blogger posts.') + '&labels=' + encodeURIComponent('blogger, widgets, free, recent, latest, posts')
}, {
	'nzw' : 'dyg',
